...
# Ines says: 
#
#    Basically, this has a class named Data, that inherits some things on
#    the file-classes that I want to read. Those log files on several
#    devices on the Perlan netword:
#
#    UNR ozone and temperature sensor logs IMU logs Air Data Probe (ADP)
#    logs Rear concentrator The most relevant one is the rear concentrator
#    log file. For many of the flights Morgan has already converted this
#    into an .xlsx spreadsheet so he must have a script, too. Those files
#    could also be used to get started on data analysis.
#
#    My code essentially does the following: Get rid of all binary
#    messages that come from devices that are not relevant (I think from
#    the IMU). That creates a "*_pro"-file that is ASCII-readable. Then,
#    this file is scanned for the message identifiers of interest, puts
#    everything into some really weird lists and then those are mashed
#    into a dataframe - I wonder why I did this so badly and not even
#    use dictionaries. Anyways, I think you can get the relevant information
#    out of it in terms of which field in a message is what.
#
...
def extract_rcdata(filename, all_dollars=False):

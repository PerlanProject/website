var FidemoSettings = function( $ ) {
	...
	var minWidth = wWidth < 950 ? wWidth : 950;
	var minHeight = wHeight < 630 ? wHeight : 630;
	var dialogHeight = wHeight < 630 ? wHeight : wHeight /1.6 ;
	var dialogWidth = (jQuery( window ).width()+ 20) /1.6;
	var dialogPosition = { my: 'center center', at: 'center center', of: window };
	...
	self.init_dialogs = function() {
		...
		self.fidemo_dialog = jQuery( '#js-fidemo-dialog-container' ).dialog( {
			...
			autoOpen: false,
			modal: true,
			width: minWidth > dialogWidth ? minWidth : dialogWidth,
			height: minHeight > dialogHeight ? minHeight : dialogHeight,
	...
	jQuery( document ).on( 'click', '.js-fidemo-refsite', function( e ) {
		...
		jQuery.each( currentSite.plugins, function( index, value ) {
			if (typeof value.title !== 'undefined') {
				...
				if ( value.only_for_theme && value.only_for_theme.length > 0 ) {
					requiredPluginsThemes += '<tr class="js-fidemo-plugins-list-theme theme-' + value.only_for_theme + ' hidden"><td><a href="' + value.url + '" target="_blank">' +
	...
	jQuery( document ).on( 'change', 'input[name="multiple-themes"]', function( e ) {
		var canInstall = true;
		var theme = jQuery( this).val();
		var wpml = jQuery( 'input[name="fidemo-wpml"]').val();

		jQuery( '.js-fidemo-plugins-list-theme').hide( 'fast', 'swing', function() {
			jQuery( '.js-fidemo-plugins-list-theme.theme-' + theme ).show();
		});
		if (wpml === 'no-wpml') {
			jQuery( '.js-fidemo-plugins-list-wpml' ).hide();
			canInstall = fidemo_check_plugins_compatibility( currentSite, false, theme );
		} else {
			jQuery( '.js-fidemo-plugins-list-wpml' ).show();
			canInstall = fidemo_check_plugins_compatibility( currentSite, true, theme );
		}
	...
	jQuery( document ).on( 'change', 'input[name="fidemo-wpml"]', function( e ) {
		var canInstall = true;
		var theme = jQuery( 'input[name="multiple-themes"]:checked' ).val();
		jQuery( '.js-fidemo-plugins-list-theme').hide( 'fast', 'swing', function() {
			jQuery( '.js-fidemo-plugins-list-theme.theme-' + theme ).show();
		});
	...
	var proccess_installation_step = function( step ) {
		...
		jQuery.ajax( {
			...
			success: function( response ) {
				...
				if (response.success) {
					jQuery( '.js-fidemo-installation-proccess li' ).eq( step ).append( ' <i class="fa fa-check" ></i>' );
	...
	function fidemo_check_plugins_compatibility( current_site, wpml, theme = '' ) {
		var canInstall = true;
		if (siteIsReady === 0) {
			return false;
		}
		...
		jQuery.each( current_site.plugins, function( index, value ) {
			...
			if (typeof value.title !== 'undefined') {
				...
				if ( value.only_for_theme && theme !== value.only_for_theme ) {
					return true;
				}
